...
function BookingForm() {
  ...
  const makeApiRequest = async (url, options) => {
    ...
    if (accessToken && isTokenExpired(accessToken)) {
        try {
            const refreshResponse = await axios.post('https://roombooking-okvk.onrender.com/refresh-token', {
...
const fetchEmails = async () => {
  try {
      const response = await makeApiRequest("https://roombooking-okvk.onrender.com/get_employee_emails", {
          method: 'GET',
  ...
  const fetchFilteredEmails = async (searchTerm) => {
    try {
        const response = await makeApiRequest(`https://roombooking-okvk.onrender.com/get_employee_emails?search=${searchTerm}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
        if (response) {
...
const processBooking = async () => {
    ...
    try {
      const response = await makeApiRequest("https://roombooking-okvk.onrender.com/book_room", {
          method: 'POST',
          data: bookingData,
          headers: { 'Content-Type': 'application/json' }
      });
